...
test.describe('VitePress Pages Accessibility', () => {
  ...
  test('Troca de idioma deve funcionar', async ({ page }) => {
    ...
    const languageSelector = page.locator('[aria-label*="language"], .VPNavBarTranslations, a[href*="pt-br"]');
  ...
  test('Busca deve estar disponível', async ({ page }) => {
    ...
    const searchButton = page.locator('button:has-text("Search"), .DocSearch, [placeholder*="Search"]');
...
test.describe('Estrutura de documentação', () => {
  ...
  test('Navegação entre páginas consecutivas deve funcionar', async ({ page }) => {
    ...
    const nextLink = page.locator('a:has-text("Next"), a:has-text("Próximo"), .pager-link.next, .prev-next a').first();
    ...
    if (count > 0) {
      ...
      const urlChanged = currentUrl !== `http://localhost:5173${initialUrl}` && 
  ...
  test('Todos os links internos devem ser válidos', async ({ page }) => {
    ...
    for (const pageUrl of pagesToCheck) {
      ...
      const links = await page.locator('a[href^="/"], a[href^="./"], a[href^="../"]').evaluateAll(
    ...
    if (brokenLinks.length > 0) {
      const errorMessage = 'Links quebrados encontrados:\n' + 
        brokenLinks.map(({ page, link, reason }) => 
          `  - Na página ${page}: ${link} (${reason})`
        ).join('\n');
